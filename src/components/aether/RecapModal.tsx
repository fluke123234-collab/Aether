          ...
          <div className="max-h-[60vh] overflow-y-auto px-7 pb-7 pt-6 aether-scroll">
            {loading ? (
              <div className="space-y-5">
                <div className="flex gap-6"><div className="h-8 w-10 animate-pulse rounded-full bg-zinc-100" /><div className="h-8 w-10 animate-pulse rounded-full bg-zinc-100" /><div className="h-8 w-10 animate-pulse rounded-full bg-zinc-100" /></div>
                <div className="space-y-2"><div className="h-3 w-24 animate-pulse rounded-full bg-purple-100" /><div className="h-4 w-full animate-pulse rounded-full bg-zinc-100" /><div className="h-4 w-5/6 animate-pulse rounded-full bg-zinc-100" /></div>
              </div>
            ) : data ? (
              <>
                {data.quiet && <div className="mb-4 flex items-center gap-2 rounded-2xl bg-zinc-50 px-4 py-2.5 text-xs font-medium text-zinc-500"><Sparkles className="h-3.5 w-3.5 text-zinc-400" />No thoughts captured in the last 24 hours.</div>}
                <div className="mb-6 flex gap-6">
                  <div><div className="font-display text-2xl leading-none text-zinc-900">{data.stats.captured}</div><div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-zinc-400">today</div></div>
                  <div><div className="font-display text-2xl leading-none text-zinc-900">{data.stats.total}</div><div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-zinc-400">kept</div></div>
                  <div><div className="font-display text-2xl leading-none text-zinc-900">{data.stats.recalled}</div><div className="mt-1 text-[10px] uppercase tracking-[0.16em] text-zinc-400">recalled</div></div>
                </div>
                <div className="mb-6">
                  ...
                  <p className="font-display text-lg leading-relaxed tracking-tight text-zinc-800">{data.distillation}</p>
                </div>
                <div>
                  <div className="mb-3 flex items-center gap-2 text-xs font-medium uppercase tracking-[0.14em] text-zinc-400"><Lightbulb className="h-3.5 w-3.5" />Quiet insights</div>
                  <ul className="space-y-2.5">
                    {data.insights.map((insight, i) => (
                      <li key={i} className="flex gap-3 rounded-xl bg-zinc-50/60 px-4 py-2.5 text-sm leading-relaxed text-zinc-600">
            ...
            ) : <div className="py-10 text-center text-sm text-zinc-400">Could not load your recap.</div>}
          </div>
        </div>
      </div>
